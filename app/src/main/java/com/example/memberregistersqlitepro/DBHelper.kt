...
class DBHelper(val context: Context?,val name: String?, val version: Int):
    ...
    fun selectLogin(id:String, password: String): Boolean {
        ...
        try {
            cursor = db.rawQuery("select id from memberTBL where id = '${id}' and password = '${password}'", null)
            if(cursor.count >= 1){
                Log.d("DBHelper", "selectLogin 로그인 성공")
                flag = true
            }
        }catch (e: SQLException){
            Log.d("DBHelper", "selectLogin ${id} 로그인 예외실패 ${e.printStackTrace()}")
        }
        return flag
    }
    ...
    fun selectID(id: String): Member?{
        ...
        try {
            ...
            if(cursor.moveToFirst()){
                Log.d("DBHelper", "selectID ${id} 데이터 정보 로드성공")
    ...
    fun updateTBL(member: Member?): Boolean{
        ...
        try {
            if(member != null) {
                db.execSQL("update memberTBL set phone = '${member?.phone}',name = '${member?.name}'," +
                        "level = '${member?.level}' where id = '${member?.id}'")
